...
export type Metric={sharpe:number;return:number;max_drawdown:number;sortino:number;position_changes:number;win_rate:number;turnover:number;active_bars?:number;exposure?:number;volatility?:number;annualized_return?:number;psr?:number};
...
export type Curve={timestamp:string;equity:number;benchmark:number;drawdown:number;return:number;signal:number;prediction_bps:number;signal_timestamp?:string;close?:number;confidence?:number};
...
export type Result={metrics:Metric;validation_metrics:Metric;validation_test_sharpe_delta:number;curve:Curve[];trades?:Record<string,unknown>[];signal_confidence?:{source:string;calibrated:boolean;scale?:number};selected_model:string;base_model?:string;selected_features:string[];parameters:Record<string,number>;optimization:Optimization;warnings:string[];split:{development:number;test:number;gap:number;test_start:string;test_end:string};feature_analysis:{features:{feature:string;ic:number;rolling_ic:number[];sign_consistency:number;mutual_information:number}[];redundancy_pairs:{a:string;b:string;correlation:number}[]};regimes:{id:number;bars:number;share:number;mean_net_return_bps:number|null}[];routing?:{enabled:boolean;fallback_model:string;min_regime_bars:number;assignments:Record<string,string>;states_source:string;specialists:{regime:number;model:string;features:string[];train_bars:number;used_fallback:boolean;fallback_model:string|null}[]};cost_sensitivity:({total_bps:number;cost_multiplier:number}&Metric)[]};
