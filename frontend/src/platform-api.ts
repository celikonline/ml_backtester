...
export type Result={metrics:Metric;validation_metrics:Metric;validation_test_sharpe_delta:number;curve:Curve[];trades?:Record<string,unknown>[];signal_confidence?:{source:string;calibrated:boolean;scale?:number};selected_model:string;selected_features:string[];parameters:Record<string,number>;optimization:Optimization;warnings:string[];split:{development:number;test:number;gap:number;test_start:string;test_end:string};feature_analysis:{features:{feature:string;ic:number;rolling_ic:number[];sign_consistency:number;mutual_information:number}[];redundancy_pairs:{a:string;b:string;correlation:number}[]};regimes:{id:number;bars:number;share:number;mean_net_return_bps:number|null}[];cost_sensitivity:({total_bps:number;cost_multiplier:number}&Metric)[]};
