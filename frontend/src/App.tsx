...
import { Activity, ArrowDownToLine, ArrowRight, ArrowUpRight, BookOpen, Calendar, Check, ChevronDown, Circle, CircleHelp, Clock3, Database, FlaskConical, Layers3, LayoutDashboard, LayoutGrid, List, Loader2, LogOut, Maximize2, Minimize2, Moon, Play, Plus, Radio, Search, Settings2, Square, Sun, Tag, Terminal, Upload, User, X, Zap } from 'lucide-react';
...
import './app/styles/methodology.css';
import './app/styles/guide-panel.css';
...
function App(){
  ...
  const [tourOpen,setTourOpen]=useState(false),[tourStep,setTourStep]=useState(0),[guideOpen,setGuideOpen]=useState(false),[guideFullscreen,setGuideFullscreen]=useState(false);
  ...
  useEffect(()=>{
    const syncTheme=(event:Event)=>{const next=(event as CustomEvent<'dark'|'light'>).detail;if(next==='dark'||next==='light')setTheme(next);};
    window.addEventListener('regimelab:theme',syncTheme);
    return()=>window.removeEventListener('regimelab:theme',syncTheme);
  },[]);
  useEffect(()=>{
    document.documentElement.classList.toggle('guide-is-pinned',guideOpen);
    return()=>document.documentElement.classList.remove('guide-is-pinned');
  },[guideOpen]);
  ...
  const heading:Record<string,string>={getstarted:t('heading.getStarted'),platform:t('heading.platform'),overview:t('heading.overview'),data:t('heading.data'),models:t('heading.models'),regimes:t('heading.regimes'),history:t('heading.history'),method:t('heading.method'),notebook:'Notebook Lab',account:t('nav.account'),'experiment-results':'Experiment Results'};
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <nav>{nav.map(([id,label,Icon])=><button key={id} data-tour={`nav-${id}`} className={page===id?'nav-item selected':'nav-item'} onClick={()=>setPage(id)}><Icon size={18}/>{label}{id==='overview'&&<span className="nav-dot"/>}</button>)}</nav>
      ...
      <div className="sidebar-bottom">
        <button data-tour="nav-method" className={`nav-item ${page==='method'?'selected':''}`} onClick={()=>setPage('method')}><BookOpen size={18}/>{t('nav.method')}</button>
      </div>
    </aside>
    <div className="main-shell">
      <header className="topbar">
        ...
        <div className="topbar-right">
          ...
          <button data-tour="help" className="icon-button" title={t('heading.getStarted')} aria-label={t('heading.getStarted')} onClick={()=>setGuideOpen(true)}><CircleHelp size={18}/></button>
    ...
    <main>
      <div className="page-heading"><div><div className="eyebrow">EUR/USD <span>·</span> {lang==='en'?'REGIME-AWARE MODELING':'REJİM ODAKLI MODELLEME'}</div><h1>{page==='overview'?t('hero.title'):heading[page]}</h1><p>{page==='overview'?t('hero.subOverview'):t('hero.subOther')}</p></div><button data-tour="new-experiment" className="primary" onClick={()=>{setError('');if(page==='platform'||page==='getstarted'){setPage('platform');setPlatformNew(n=>n+1);}else setModal(true);}} disabled={page!=='platform'&&page!=='getstarted'&&active}><Plus size={17}/>{t('action.newExperiment')}</button></div>
      ...
      {page==='getstarted'&&<GettingStarted onNavigate={setPage} onNewExperiment={()=>{setPage('platform');setPlatformNew(n=>n+1);}} onDemo={()=>{setPage('platform');setPlatformNew(n=>n+1);}} onStartTour={()=>{setTourStep(0);setTourOpen(true);}}/>}
      {page==='overview'&&<>
      ...
      {page==='method'&&<div className="method-layout"><section className="panel prose"><span className="eyebrow">{t('method.kicker')}</span><h2>{t('method.title')}</h2><p>{t('method.intro')}</p><div className="method-guide"><h3>{t('method.guideTitle')}</h3><p>{t('method.guideIntro')}</p><ol className="method-guide-list"><li>{t('method.guide1')}</li><li>{t('method.guide2')}</li><li>{t('method.guide3')}</li><li>{t('method.guide4')}</li><li>{t('method.guide5')}</li></ol></div><h3>{t('method.regimeTitle')}</h3><p>{t('method.regimeText')}</p><h3>{t('method.notebooks')}</h3>{project?.notebooks.map(n=><div className="notebook" key={n.name}><BookOpen size={20}/><div><b>{n.name}</b><p>{n.name.startsWith('optimus')?t('method.nbOptimus'):t('method.nbTez')}</p></div><span>{n.cells} {t('method.cells')}</span></div>)}<h3>{t('method.flow')}</h3><p>{project?.scope}</p><ol>{(result?.notes||[t('method.note1'),t('method.note2'),t('method.note3'),t('method.note4'),t('method.note5')]).map(n=><li key={n}>{n}</li>)}</ol><h3>{t('method.findings')}</h3><p>{t('method.findingsText')}</p><p>{t('method.sharpe')}</p><div className="inline-note">{t('method.synthetic')}</div></section><section className="panel method-side"><h2>{t('method.protocol')}</h2><div><small>{t('method.goal')}</small><b>{t('method.goalV')}</b></div><div><small>{t('method.val')}</small><b>{t('method.valV')}</b></div><div><small>{t('method.gap')}</small><b>{t('method.gapV')}</b></div><div><small>{t('method.regime')}</small><b>{t('method.regimeV')}</b></div><div><small>{t('method.seed')}</small><b>42</b></div><div><small>{t('method.cost')}</small><b>{t('method.costV')}</b></div></section><section className="panel capability-panel"><h2>Özellik kapsamı ve gelecek planı</h2><p className="table-note">Durumlar mevcut kod ve veri sözleşmesine göre işaretlenmiştir. “Kısmen var”, temel mekanizmanın bulunduğunu ancak üretim seviyesinde tam kapsama ulaşmadığını belirtir.</p>{capabilityTables.map(table=><div className="capability-group" key={table.title}><h3>{table.title}</h3><div className="capability-table-wrap"><table className="capability-table"><thead><tr><th>Özellik</th><th>Açıklama</th><th>Durum</th><th>Gelecek planı</th></tr></thead><tbody>{table.rows.map(row=><tr key={row.name}><td><b>{row.name}</b></td><td>{row.explanation}</td><td><span className={`cap-status ${row.status.replace(' ','-').toLowerCase()}`}>{row.status}</span></td><td>{row.plan}</td></tr>)}</tbody></table></div></div>)}<p className="table-note">{t('cap.summary')}</p></section></div>}
      {guideOpen&&<aside className={`guide-panel ${guideFullscreen?'is-fullscreen':''}`} aria-label={t('heading.getStarted')}><div className="guide-panel-header"><div><span className="eyebrow">{t('getStarted.kicker')}</span><h2>{t('heading.getStarted')}</h2></div><div className="guide-panel-header-actions"><button className="icon-button" title={guideFullscreen?(lang==='en'?'Exit full screen':'Tam ekrandan çık'):(lang==='en'?'Full screen':'Tam ekran')} aria-label={guideFullscreen?(lang==='en'?'Exit full screen':'Tam ekrandan çık'):(lang==='en'?'Full screen':'Tam ekran')} onClick={()=>setGuideFullscreen(value=>!value)}>{guideFullscreen?<Minimize2 size={18}/>:<Maximize2 size={18}/>}</button><button className="icon-button" aria-label={t('alert.closeError')} onClick={()=>{setGuideOpen(false);setGuideFullscreen(false);}}><X size={18}/></button></div></div><div className="guide-panel-body"><GettingStarted onNavigate={pageName=>setPage(pageName)} onNewExperiment={()=>{setPage('platform');setPlatformNew(n=>n+1);}} onDemo={()=>{setPage('platform');setPlatformNew(n=>n+1);}} onStartTour={()=>{setGuideOpen(false);setGuideFullscreen(false);setTourStep(0);setTourOpen(true);}}/></div></aside>}
      <footer className="footer"><span><Activity size={13}/>REGIME LAB <i/> {t('footer.built')}</span><span>{t('footer.local')}</span></footer>
      <ChatBot />
      <ProductTour open={tourOpen} stepIndex={tourStep} onStepChange={setTourStep} onClose={()=>setTourOpen(false)} onNavigate={setPage}/>
    </main></div>
